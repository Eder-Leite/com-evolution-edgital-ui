...
import { SchemaCarteiraFinanceira, empresa } from '../TesModel';
const url = 'carteirasFinanceira';
...
function CarteiraFinanceira() {
    ...
    const [carteira, setCarteira] = useState(new SchemaCarteiraFinanceira());

    useEffect(() => {
        document.title = 'Evolution Sistemas - eDigital | Carteira Financeira';
    }, []);
    ...
    async function pesquisar(eventPage = 0, eventRows = 0, eventFirst = 0) {
        ...
        await Api({
            ...
            setFirst(eventFirst);
            setCarteiras(resp.data.content);
    ...
    async function salvar() {
        ...
        if (carteira.id > 0) {
            await Api({
                method: 'put',
                url: `${url}/${carteira.id}`,
                data: JSON.stringify(carteira)
            }).then(resp => {
                Loading.onHide();
                cancelar();
                pesquisar();
                Toasty.success('Sucesso!', 'Carteira Financera editada com sucesso!');
            })
                .catch(error => {
                    Loading.onHide();
                    console.log(error);
            ...
            await Api({
                ...
                cancelar();
                pesquisar();
                Toasty.success('Sucesso!', 'Carteira Financeira adicionada com sucesso!');
            })
                .catch(error => {
                    Loading.onHide();
                    console.log(error);
    ...
    function validaFormulario() {
        try {
            if (carteira.descricao.length >= 3) {
                return true;
            } else {
                return false;
            }
    ...
    function inserir() {
        setActiveIndex(1);
        setCarteira(new SchemaCarteiraFinanceira());
    }
    ...
    const header = <div className='p-clearfix' style={{ lineHeight: '1.87em' }}>Lista de Carteiras Financeira</div>;
                    ...
                    <Card>
                        <TabView activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)}>
                            ...
                            <TabPanel disabled header='Cadastro'>
                                <h2 style={{ margin: 5 }}>Cadastro de Carteira Financeira</h2>
                                <div className='p-fluid'>
                                    <div className='p-col-12'>
                                        <div className='p-grid'>
                                            ...
                                            <div className='p-col-12 p-md-12'>
                                                ...
                                                <Dropdown
                                                    ...
                                                    filterBy='label,value'
                                                    value={carteira.tipo}
                                                    onChange={(e) => setCarteira({ ...carteira, tipo: e.value })}
                                                />
                                            </div>
                                            <div className='p-col-12 p-md-12'>
                                                <label htmlFor='in' style={{ fontWeight: 'bold' }}>Descrição *</label>
...
export default withRouter(CarteiraFinanceira);