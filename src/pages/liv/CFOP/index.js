...
import { SchemaCFOP, empresa } from '../LivModel';
const url = 'codigosCFOP';
...
function CFOP() {
    ...
    const [cfop, setCfop] = useState(new SchemaCFOP());

    useEffect(() => {
        document.title = 'Evolution Sistemas - eDigital | CFOP';
    }, []);
    ...
    async function salvar() {
        ...
        if (cfop.id > 0) {
            await Api({
                method: 'put',
                url: `${url}/${cfop.id}`,
                data: JSON.stringify(cfop)
            }).then(resp => {
                Loading.onHide();
                cancelar();
                pesquisar();
                Toasty.success('Sucesso!', 'CFOP editado com sucesso!');
            })
                .catch(error => {
                    Loading.onHide();
                    console.log(error);
            ...
            await Api({
                ...
                cancelar();
                pesquisar();
                Toasty.success('Sucesso!', 'CFOP adicionado com sucesso!');
            })
                .catch(error => {
                    Loading.onHide();
                    console.log(error);
    ...
    function validaFormulario() {
        try {
            if (cfop.codigo.length === 4 &&
                cfop.descricao.length >= 4 &&
                cfop.tipo) {
                return true;
            } else {
                return false;
    ...
    const header = <div className='p-clearfix' style={{ lineHeight: '1.87em' }}>Lista de CFOPs</div>;
                    ...
                    <Card>
                        <TabView activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)}>
                            <TabPanel disabled header='Lista'>
                                ...
                                <div className='content-section implementation'>
                                    ...
                                    >
                                        ...
                                        <Column field='tipo' header='Tipo Movimento' style={{ width: '12em' }} />
                            ...
                            <TabPanel disabled header='Cadastro'>
                                <h2 style={{ margin: 5 }}>Cadastro de CFOP</h2>
                                <div className='p-col-12 p-md-2'>
                                ...
                                <div className='p-col-12 p-md-12'>
                                    ...
                                    <InputText
                                        ...
                                        value={cfop.codigo}
                                        keyfilter={/[0-9]+$/}
                                        readOnly={cfop.id ? true : false}
                                        onChange={(e) => setCfop({ ...cfop, codigo: e.target.value })}
                                    />
                                </div>
                                <div className='p-col-12 p-md-12'>
                                    ...
                                    <InputTextarea
                                        ...
                                        onChange={(e) => setCfop({ ...cfop, descricao: e.target.value })}
                                    />
                                </div>
                                <div className='p-col-12 p-md-12'>
                                    <label htmlFor='in' style={{ fontWeight: 'bold' }}>Tipo Movimentção *</label>
                                    <Dropdown
                                        filter={true}
                                        options={tipos}
                                        value={cfop.tipo}
                                        filterBy='label,value'
                                        onChange={(e) => setCfop({ ...cfop, tipo: e.value })}
                                    />
                                </div>
                                <div className='p-col-12 p-md-12'>
                                    <Toolbar>
...
export default withRouter(CFOP);